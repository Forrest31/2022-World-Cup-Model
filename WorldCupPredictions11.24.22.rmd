...
```{r Create Test Data}
group_stage_matches<- read_csv("22 World Cup Group Stage Test Set 11.24.22.csv", show_col_types = FALSE)

group_stage_matches$Outcome<-rep(4)

df_test_y = group_stage_matches %>% pull("Outcome") 
df_test_x = group_stage_matches %>% select(-c(("Outcome")))

all_test<-group_stage_matches
...
group_stage_prediction <- matrix(xgbpred, nrow = numberofClasses,
...
```{r}
group_stage_prediction<-cbind(group_stage_matches[1:2],group_stage_prediction)

group_stage_prediction<-group_stage_prediction[complete.cases(group_stage_prediction), ]

colnames(group_stage_prediction)<-c("Home", "Away", "HmModelProb", "VisModelProb", "DrawModelProb", "Prediction")

group_stage_prediction<-group_stage_prediction[order(group_stage_prediction$Home), ]
 
```

```{r}
group_stage_odds <- read_csv("22 World Cup Group Stage Odds 11.24.csv", show_col_types = FALSE)

group_stage_odds<- group_stage_odds[order(group_stage_odds$Home), ]
```

```{r}
final_pred<-cbind(group_stage_prediction, group_stage_odds[9:11])
write.csv(final_pred, "WorldCup22GroupStagePrediction11.24.22.csv")
```
