...
1. Outcomes of international friendly, world cup qualifying, and world cup tournament matches between 2010-2022
...
### Data Cleaning 
#### Consistency
The biggest challenge was creating consistentency country names as they are abbreviated differently in each dataset.  Additionally, numerous countries changed their names over the 12-year period of investigation. 

#### Joining Data
Outcome data needed to be joined with rating data. Next teams needed to be assigned a unique game ID. meaning the two teams playing one another will have the same Gamne ID. Next the teams are split based on being home or away to then be joined based on the unique ID to create a match as a single observation. 

### Feature Engineering
In addition to the collected data, I added variables to measure:
...
### Model Performance
After fitting the model, it is validated on a random selection of games of over 2000 games between 2010-2022.  The overall accuracy is 57%. With 95% confidence, we can expect the overall accuracy to fall between 55 and 59 percent. 
